...
ExplicitSchemeRuntimeSchedule::ExplicitSchemeRuntimeSchedule(const InputFile* input, Mesh* m) :
    mesh(m)
{
    // int nx = mesh->getNx()[0];
    // int ny = mesh->getNx()[1];
...
void ExplicitSchemeRuntimeSchedule::doAdvance(const double dt)
{
...
void ExplicitSchemeRuntimeSchedule::updateBoundaries()
{
    reflectBoundaries();    
}

void ExplicitSchemeRuntimeSchedule::init()
{
    updateBoundaries();
}

void ExplicitSchemeRuntimeSchedule::reset()
{
    double start = omp_get_wtime();
...
//    #pragma omp parallel for firstprivate(u0, u1, x_min, x_max, y_min, y_max, nx) schedule(runtime)
...
//        }
//    }
    #pragma omp parallel for firstprivate(x_min, x_max, y_min, y_max, nx) schedule(runtime)
    for(int k = y_min-1; k <= y_max+1; k++) {
    ...
    // int *n = mesh->getNx();
    // #pragma omp parallel for firstprivate(n, x_min, x_max, y_min, y_max, nx) schedule(runtime)
...
void ExplicitSchemeRuntimeSchedule::diffuse(double dt)
{
    double* u0 = mesh->getU0();
    double* u1 = mesh->getU1();
    int x_min = mesh->getMin()[0];
...
//    int k, j;
    ...
    #pragma omp parallel for firstprivate(nx, y_max, y_min, x_min, x_max, ry, rx) schedule(runtime)
...
void ExplicitSchemeRuntimeSchedule::reflectBoundaries()
{
    double* u0 = mesh->getU0();
    int x_min = mesh->getMin()[0];
    int x_max = mesh->getMax()[0];
